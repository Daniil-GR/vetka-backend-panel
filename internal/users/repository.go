...
func (r *Repository) AccessByID(ctx context.Context, userID, accessID string) (Access, error) {
	rows, err := r.pool.Query(ctx, `select id, user_id, node_id, protocol_type, protocol_username, protocol_password, enabled, created_at, updated_at from user_node_access where user_id=$1 and id=$2`, userID, accessID)
	if err != nil {
		return Access{}, err
	}
	defer rows.Close()
	return pgx.CollectOneRow(rows, scanAccess)
}

func (r *Repository) SetAccessEnabled(ctx context.Context, userID, accessID string, enabled bool) error {
	_, err := r.pool.Exec(ctx, `update user_node_access set enabled=$3 where user_id=$1 and id=$2`, userID, accessID, enabled)
	return err
}
...
func (r *Repository) ActiveAccessForNode(ctx context.Context, nodeID string) ([]AccessWithUser, error) {
	rows, err := r.pool.Query(ctx, `select a.id, a.user_id, a.node_id, a.protocol_type, a.protocol_username, a.protocol_password, a.enabled, a.created_at, a.updated_at, u.username, u.enabled, u.expires_at, u.quota_mb
...
order by u.username`, nodeID)
	...
	return pgx.CollectRows(rows, func(row pgx.CollectableRow) (AccessWithUser, error) {
		...
		err := row.Scan(&a.ID, &a.UserID, &a.NodeID, &a.ProtocolType, &a.ProtocolUsername, &a.ProtocolPassword, &a.Enabled, &a.CreatedAt, &a.UpdatedAt, &a.Username, &a.UserEnabled, &a.UserExpiresAt, &a.UserQuotaMB)
		return a, err
	})
}
